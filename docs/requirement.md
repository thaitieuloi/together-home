...
| **Map History V2** | Tự động tách Chuyến đi/Điểm dừng, Playback | ✅ (Premium UI) | 🏗️ (Đang làm) | Web có UI Glassmorphism, Filters & Non-modal |
| **Chi tiết Thành viên** | Xem địa chỉ, pin, tốc độ của người khác | ✅ | ✅ | Web dùng Action Sheet / Mobile dùng bottom sheet |
| **Marker Clustering** | Nhóm các thành viên ở gần nhau | ✅ (Cluster Group) | 🏗️ (Sắp tới) | Tối ưu hiệu năng khi có nhiều thành viên |
| **Navigation Picker** | Mở Google/Apple Maps để chỉ đường | ✅ | 🏗️ (Sắp tới) | Hỗ trợ chọn ứng dụng bản đồ ưa thích |
| **Lazy Loading** | Chỉ tải Avatar khi cuộn đến | ✅ | ✅ | Tiết kiệm băng thông & tăng tốc UI |
| **Battery Alerts** | Thông báo khi pin thành viên dưới 20% | ✅ (In-app) | ✅ (Edge Func) | Cooldown 30p để tránh spam thông báo |
...
*   **Web (New Standard)**: 
    ...
    *   **Activity & Date Filters**: Bổ sung bộ lọc theo loại hoạt động (Lái xe, Đi bộ, Đạp xe, Dừng chân) và chọn khoảng ngày (Custom Date Range) để tra cứu dữ liệu cũ.
*   **Mobile (Todo)**: 
...
### 5. Nhất quán tên gọi & Logic Trạng thái
*   Sử dụng chung thuật ngữ: "Vừa xong" (Recently/Idle) thay vì "Hoạt động".
*   **Quy tắc 5 phút**: Trạng thái "Vừa xong" (Just Now) được mở rộng cho mọi truy cập trong vòng **<= 5 phút** (thay vì 1 phút) để phản ánh đúng thực tế độ trễ GPS và tiết kiệm pin thiết bị.
*   Sắp xếp danh sách thành viên theo bảng chữ cái để dễ tìm kiếm khi gia đình có đông người.

### 6. Tối ưu hóa Bản đồ & Danh sách
*   **Marker Cluster Group**: Web App sử dụng kỹ thuật clustering để nhóm các thành viên đứng gần nhau. Khi phóng to (`zoom >= 16`), các điểm sẽ tự động tách ra.
*   **Lazy Avatar List**: Danh sách sidebar sử dụng `IntersectionObserver` để chỉ kích hoạt tải ảnh đại diện khi người dùng cuộn tới, giảm tải cho CPU và mạng.
*   **Navigation Bridge**: Khi cần chỉ đường, người dùng có thể chọn mở nhanh tọa độ thành viên qua Google Maps hoặc Apple Maps.
...
3.  **Advanced Route Analytics (Phân tích lộ trình)**:
